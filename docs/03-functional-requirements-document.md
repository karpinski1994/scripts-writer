...
| FR-1.6 | The system shall allow the user to attach existing ICP data from upload or Piragi if available, skipping the ICP agent | Should |

### FR-2: ICP Analysis Agent

| ID | Requirement | Priority |
|----|------------|----------|
| FR-2.1 | The system shall analyze raw notes (optionally enriched by Piragi context) and generate an ICP profile including demographics, psychographics, pain points, and desires | Must |
...
### FR-15: Piragi RAG Integration

| ID | Requirement | Priority |
|----|------------|----------|
| FR-15.1 | The system shall allow the user to connect Piragi RAG documents to a project by specifying document paths | Must |
| FR-15.2 | The system shall allow the user to attach Piragi context to any creative pipeline step before running the agent | Must |
| FR-15.3 | The system shall query the Piragi documents for insights relevant to the current step type (ICP profiling, hook ideas, narrative summaries, retention techniques, CTA wording, script generation) | Should |
| FR-15.4 | The system shall present Piragi-sourced context alongside agent output for review | Should |
| FR-15.5 | The system shall degrade gracefully if Piragi is unavailable — agents proceed with raw notes only | Must |
| FR-15.6 | The system shall allow the user to disconnect RAG documents from a project | Should |
